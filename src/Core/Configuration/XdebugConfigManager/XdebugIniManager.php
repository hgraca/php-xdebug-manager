...
final class XdebugIniManager extends XdebugConfigManagerAbstract
{
    ...
    {
        file_put_contents($this->getContext()->getXdebugIniPath(), $iniConfig);
    }

    protected function readIniConfig(): array
    {
        return file($this->getContext()->getXdebugIniPath());
    }
...
xdebug.remote_host=${hostIp}       ;# Host where the debug client is running, you can either use a host name, IP address, or 'unix:///path/to/sock' for a Unix domain socket. This setting is ignored if xdebug.remote_connect_back is enabled.
